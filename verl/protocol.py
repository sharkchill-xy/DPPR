...
class DataProto:
    ...
    def slice(self, indices) -> 'DataProto':
        """
        Slice the batch and non_tensor_batch according to the indices.
        """
        tensor_data = self.batch[indices]
        non_tensor_data = {key: val[indices] for key, val in self.non_tensor_batch.items()}
