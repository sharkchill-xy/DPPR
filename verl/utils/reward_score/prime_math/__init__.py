...
def _last_boxed_only_string_depracted(string):
...
def _last_boxed_only_string(string):
    ...
    if idx < 0:
        idx = string.rfind("\\fbox")
        if idx < 0:
            # If neither \boxed nor \fbox is found, check for <answer>
            idx = string.rfind("<answer>")
            if idx < 0:
                return None  # Return None if none of the patterns are found

    if "\\boxed" in string[idx:] or "\\fbox" in string[idx:]:
        i = idx
        left_brace_idx = None
        right_brace_idx = None
        num_left_braces_open = 0
        ...
        # Handle <answer> and </answer>
        end_idx = string.rfind("</answer>", idx)
        if end_idx < 0:
            return None  # Return None if </answer> tag is not found

        start_content_idx = idx + len("<answer>")
        return string[start_content_idx:end_idx].strip()



def match_answer(response, version='1'):
    is_matched = False
    for ans_marker in ['answer:', "answer is", "answers are", "is approximately"]:
    ...
    if version == '1':
        ...
        if response.endswith('.'): # conside the case of $7.71$.
...
def direct_match_mcq(output, gt):
    end = r')[\s\.\}]'
    patterns = [
        r'boxed\{\s*(' + re.escape(gt) + r')\s*\}',
        r'<answer>\s*(' + re.escape(gt) + r')\s*</answer>',
        r'ANSWER:\s*(' +  re.escape(gt) + r')',
        r'he answer to the question is:?\s*(' + re.escape(gt) + r')',
        r'he correct option is:?\s*(' + re.escape(gt) + r')',
        r'he correct answer is:?\s*(' + re.escape(gt) + r')',
        r'he correct answer is:?\s*\((' + re.escape(gt) + r')\)',
    ]
...
def compute_score(model_output: str, ground_truth: str) -> bool:
    ...
    direct_match_result = direct_match_mcq(model_output, gt=ground_truth)
    if direct_match_result and direct_match_result[0]:
        return direct_match_result



    extracted_model_output_list = []
    extracted_model_output_list.append(match_answer(model_output, version='1')[1])
    extracted_model_output_list.append(match_answer(model_output, version='2')[1])
