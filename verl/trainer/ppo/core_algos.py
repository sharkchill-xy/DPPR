...
def compute_grpo_outcome_advantage(token_level_rewards: torch.Tensor,
    ...
    # Calculate filter rate and create stds tensor
    filtered_count = 0
...
def compute_prnostd_vrstd_outcome_advantage(token_level_pr: torch.Tensor, token_level_vr: torch.Tensor,
                                   pr_weight: float, vr_weight: float,
                                   eos_mask: torch.Tensor,
                                   index: torch.Tensor,
                                   std_filter: float = 1e-6,
                                   epsilon: float = 1e-6):
    ...
    (with only one scalar reward for each response).
    Args:
        token_level_vr: `(torch.Tensor)`
            shape: (bs * n_rollouts, response_length)
        token_level_pr: `(torch.Tensor)`
            shape: (bs * n_rollouts, response_length)
        eos_mask: `(torch.Tensor)`
            shape: (bs * n_rollouts, response_length)
    ...
    Returns:
        ...
        Returns: `(torch.Tensor)`
            shape: (bs * n_rollouts, response_length)
    """
    response_length = token_level_pr.shape[-1]
    pr_scores = token_level_pr.sum(dim=-1)
    vr_scores = token_level_vr.sum(dim=-1)
    final_scores = torch.zeros_like(pr_scores)

    # Calculate filter rate and create stds tensor
    # filtered_count = 0
    vr_stds = torch.zeros_like(pr_scores)

    id2pr_score = defaultdict(list)
    id2vr_score = defaultdict(list)

    with torch.no_grad():
        bsz = pr_scores.shape[0]
        for i in range(bsz):
            id2pr_score[index[i]].append(pr_scores[i])
            id2vr_score[index[i]].append(vr_scores[i])

        id2pr_mean, _ = get_reward_mean_and_std(id2pr_score)
        id2vr_mean, id2vr_std = get_reward_mean_and_std(id2vr_score)
...
def compute_prstd_vrstd_outcome_advantage(token_level_pr: torch.Tensor, token_level_vr: torch.Tensor,
                                   pr_weight: float, vr_weight: float,
                                   eos_mask: torch.Tensor,
                                   index: torch.Tensor,
                                   std_filter: float = 1e-6,
    ...
    # Calculate filter rate and create stds tensor
    # filtered_count = 0
    pr_stds = torch.zeros_like(pr_scores)
    vr_stds = torch.zeros_like(vr_scores)
    ...
    with torch.no_grad():
        ...
        id2pr_mean, id2pr_std = get_reward_mean_and_std(id2pr_score)
        id2vr_mean, id2vr_std = get_reward_mean_and_std(id2vr_score)
...
# NOTE(sgm): this implementation only consider outcome supervision, where the reward is a scalar.
def compute_grpo_nostd_outcome_advantage(token_level_rewards: torch.Tensor,
                                   eos_mask: torch.Tensor,
                                   index: torch.Tensor,
                                   std_filter: float = 1e-6,
                                   epsilon: float = 1e-6):
...
def compute_grpo_nostd_outcome_advantage_old(token_level_rewards: torch.Tensor,
                                   eos_mask: torch.Tensor,
                                   index: torch.Tensor,
                                   epsilon: float = 1e-6):
    """
    ...
    Returns:
        ...
        Returns: `(torch.Tensor)`
            shape: (bs, response_length)
    """
    print(f"We are in compute_grpo_nostd_outcome_advantage")
    response_length = token_level_rewards.shape[-1]
    scores = token_level_rewards.sum(dim=-1)
...
def agg_loss(loss_mat: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode='token-mean', max_tokens=None):
    """
    ...
    if loss_agg_mode == "token-mean":
        ...
        loss = torch.mean(seq_losses) / max_tokens # seq-mean, 2048 is the max length of the sequence
...
def compute_policy_loss(old_log_prob, log_prob, advantages, eos_mask, cliprange, loss_agg_mode='token-mean', max_tokens=None):
